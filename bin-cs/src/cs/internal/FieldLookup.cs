// Generated by Haxe 3.4.0

#pragma warning disable 109, 114, 219, 429, 168, 162
namespace haxe.lang {
	public sealed class FieldHashConflict {
		
		public FieldHashConflict(int hash, string name, object @value, global::haxe.lang.FieldHashConflict next) {
			#line 32 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
			this.hash = hash;
			this.name = name;
			#line 34 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
			this.@value = @value;
			this.next = next;
		}
		#line default
		
		public readonly int hash;
		
		public readonly string name;
		
		public object @value;
		
		public global::haxe.lang.FieldHashConflict next;
		
	}
}



#pragma warning disable 109, 114, 219, 429, 168, 162
namespace haxe.lang {
	public sealed class FieldLookup {
		
		#pragma warning disable 628
		static FieldLookup() {
			#line 50 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
			global::haxe.lang.FieldLookup.length = ( global::haxe.lang.FieldLookup.fieldIds as global::System.Array ).Length;
		}
		#line default
		
		public FieldLookup() {
		}
		
		
		protected static int[] fieldIds = new int[]{105, 4745537, 4849249, 5393365, 5442204, 5594513, 57476627, 76061764, 87367608, 302979532, 328878574, 359333139, 407283053, 452737314, 501039929, 520590566, 946786476, 1067353468, 1103412149, 1181037546, 1204816148, 1224901875, 1228056441, 1247875546, 1280845662, 1282943179, 1352786672, 1398464674, 1431819701, 1436822557, 1532710347, 1547539107, 1620824029, 1621420777, 1623148745, 1648581351, 1705629508, 1768617329, 1830310359, 1915412854, 1916009602, 1981972957, 2025055113, 2054707467, 2082663554, 2127021138};
		
		protected static string[] fields = new string[]{"i", "__a", "arr", "len", "map", "pop", "getEnumConstructs", "remove", "filter", "methodName", "iterator", "lastIndexOf", "hasNext", "reverse", "insert", "length", "toString", "splice", "copy", "join", "concat", "next", "enumConstructor", "push", "sort", "quicksort", "spliceVoid", "getEnumName", "createEnumIndex", "createEnum", "concatNative", "className", "__unsafe_get", "__unsafe_set", "indexOf", "fileName", "toDynamic", "allEnums", "customParams", "__get", "__set", "lineNumber", "unshift", "enumParameters", "shift", "slice"};
		
		protected static int length;
		
		public static void addFields(int[] nids, string[] nfields) {
			unchecked {
				#line 56 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
				int[] cids = global::haxe.lang.FieldLookup.fieldIds;
				#line 56 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
				string[] cfields = global::haxe.lang.FieldLookup.fields;
				#line 58 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
				int nlen = ( nids as global::System.Array ).Length;
				int clen = global::haxe.lang.FieldLookup.length;
				#line 60 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
				if (( ( nfields as global::System.Array ).Length != nlen )) {
					#line 60 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
					throw global::haxe.lang.HaxeException.wrap(global::haxe.lang.Runtime.concat(global::haxe.lang.Runtime.concat(global::haxe.lang.Runtime.concat("Different fields length: ", global::haxe.lang.Runtime.toString(nlen)), " and "), global::haxe.lang.Runtime.toString(( nfields as global::System.Array ).Length)));
				}
				
				#line 63 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
				bool needsChange = false;
				{
					#line 53 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
					int[] _g_arr = nids;
					#line 53 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
					uint _g_idx = ((uint) (0) );
					#line 64 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
					while (((bool) (( _g_idx < ( _g_arr as global::System.Array ).Length )) )) {
						#line 66 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
						if (( global::haxe.lang.FieldLookup.findHash(((int) (_g_arr[((int) (_g_idx++) )]) ), cids, clen) < 0 )) {
							#line 68 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
							needsChange = true;
							break;
						}
						
					}
					
				}
				
				#line 74 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
				if (needsChange) {
					#line 76 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
					lock(typeof(global::haxe.lang.FieldLookup)){
						#line 78 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
						int[] ansIds = new int[( clen + nlen )];
						#line 78 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
						string[] ansFields = new string[( clen + nlen )];
						#line 80 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
						int ci = 0;
						#line 80 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
						int ni = 0;
						#line 80 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
						int ansi = 0;
						while (true) {
							#line 81 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
							if ( ! ((( (( ci < clen )) ? (( ni < nlen )) : (false) ))) ) {
								#line 81 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
								break;
							}
							
							#line 83 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
							if (( cids[ci] < nids[ni] )) {
								#line 85 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
								ansIds[ansi] = cids[ci];
								ansFields[ansi] = cfields[ci];
								#line 87 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
								ci = ( ci + 1 );
							}
							else {
								#line 89 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
								ansIds[ansi] = nids[ni];
								ansFields[ansi] = nfields[ni];
								#line 91 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
								ni = ( ni + 1 );
							}
							
							#line 93 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
							ansi = ( ansi + 1 );
						}
						
						#line 96 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
						if (( ci < clen )) {
							#line 98 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
							global::System.Array.Copy(((global::System.Array) (cids) ), ((int) (ci) ), ((global::System.Array) (ansIds) ), ((int) (ansi) ), ((int) (( clen - ci )) ));
							global::System.Array.Copy(((global::System.Array) (cfields) ), ((int) (ci) ), ((global::System.Array) (ansFields) ), ((int) (ansi) ), ((int) (( clen - ci )) ));
							#line 100 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
							ansi = ( ansi + (( clen - ci )) );
						}
						
						#line 103 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
						if (( ni < nlen )) {
							#line 105 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
							global::System.Array.Copy(((global::System.Array) (nids) ), ((int) (ni) ), ((global::System.Array) (ansIds) ), ((int) (ansi) ), ((int) (( nlen - ni )) ));
							global::System.Array.Copy(((global::System.Array) (nfields) ), ((int) (ni) ), ((global::System.Array) (ansFields) ), ((int) (ansi) ), ((int) (( nlen - ni )) ));
							#line 107 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
							ansi = ( ansi + (( nlen - ni )) );
						}
						
						#line 111 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
						global::haxe.lang.FieldLookup.fieldIds = ansIds;
						global::haxe.lang.FieldLookup.fields = ansFields;
						#line 113 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
						global::haxe.lang.FieldLookup.length = ansi;
					}
					;
				}
				
			}
			#line default
		}
		
		
		public static int doHash(string s) {
			unchecked {
				#line 121 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
				int acc = 0;
				{
					#line 122 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
					int _g1 = 0;
					#line 122 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
					int _g = s.Length;
					#line 122 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
					while (( _g1 < _g )) {
						#line 124 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
						acc = ( ( ( 223 * (( acc >> 1 )) ) + ((int) (s[_g1++]) ) ) << 1 );
					}
					
				}
				
				#line 127 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
				return ((int) (( ((uint) (acc) ) >> 1 )) );
			}
			#line default
		}
		
		
		public static string lookupHash(int key) {
			unchecked {
				#line 133 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
				int[] ids = global::haxe.lang.FieldLookup.fieldIds;
				int min = 0;
				#line 135 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
				int max = global::haxe.lang.FieldLookup.length;
				#line 137 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
				while (( min < max )) {
					#line 139 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
					int mid = ( min + ( (( max - min )) / 2 ) );
					int imid = ids[mid];
					#line 141 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
					if (( key < imid )) {
						#line 143 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
						max = mid;
					}
					else if (( key > imid )) {
						#line 145 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
						min = ( mid + 1 );
					}
					else {
						#line 147 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
						return global::haxe.lang.FieldLookup.fields[mid];
					}
					
				}
				
				#line 151 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
				throw global::haxe.lang.HaxeException.wrap(global::haxe.lang.Runtime.concat("Field not found for hash ", global::haxe.lang.Runtime.toString(key)));
			}
			#line default
		}
		
		
		public static int hash(string s) {
			unchecked {
				#line 156 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
				if (string.Equals(s, null)) {
					#line 156 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
					return 0;
				}
				
				#line 158 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
				int acc = 0;
				#line 158 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
				{
					#line 158 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
					int _g1 = 0;
					#line 158 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
					int _g = s.Length;
					#line 158 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
					while (( _g1 < _g )) {
						#line 158 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
						acc = ( ( ( 223 * (( acc >> 1 )) ) + ((int) (s[_g1++]) ) ) << 1 );
					}
					
				}
				
				#line 158 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
				int key = ((int) (( ((uint) (acc) ) >> 1 )) );
				#line 161 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
				int[] ids = global::haxe.lang.FieldLookup.fieldIds;
				#line 161 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
				string[] fld = global::haxe.lang.FieldLookup.fields;
				#line 163 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
				int min = 0;
				int max = global::haxe.lang.FieldLookup.length;
				#line 166 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
				int len = global::haxe.lang.FieldLookup.length;
				#line 168 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
				while (( min < max )) {
					#line 170 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
					int mid = ((int) (( min + ( ((double) ((( max - min ))) ) / 2 ) )) );
					int imid = ids[mid];
					#line 172 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
					if (( key < imid )) {
						#line 174 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
						max = mid;
					}
					else if (( key > imid )) {
						#line 176 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
						min = ( mid + 1 );
					}
					else {
						#line 179 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
						if ( ! (string.Equals(fld[mid], s)) ) {
							#line 180 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
							return  ~ (key) ;
						}
						
						#line 181 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
						return key;
					}
					
				}
				
				#line 187 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
				lock(typeof(global::haxe.lang.FieldLookup)){
					#line 188 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
					if (( len != global::haxe.lang.FieldLookup.length )) {
						#line 189 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
						return global::haxe.lang.FieldLookup.hash(s);
					}
					
					#line 195 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
					global::haxe.lang.FieldLookup.insert<int>(ref global::haxe.lang.FieldLookup.fieldIds, ((int) (global::haxe.lang.FieldLookup.length) ), ((int) (min) ), ((int) (key) ));
					global::haxe.lang.FieldLookup.insert<string>(ref global::haxe.lang.FieldLookup.fields, ((int) (global::haxe.lang.FieldLookup.length) ), ((int) (min) ), ((string) (s) ));
					#line 200 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
					 ++ global::haxe.lang.FieldLookup.length;
				}
				;
				#line 202 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
				return key;
			}
			#line default
		}
		
		
		public static int findHash(int hash, int[] hashs, int length) {
			unchecked {
				#line 207 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
				int min = 0;
				int max = length;
				#line 210 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
				while (( min < max )) {
					#line 212 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
					int mid = ( (( max + min )) / 2 );
					int imid = hashs[mid];
					#line 214 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
					if (( hash < imid )) {
						#line 216 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
						max = mid;
					}
					else if (( hash > imid )) {
						#line 218 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
						min = ( mid + 1 );
					}
					else {
						#line 220 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
						return mid;
					}
					
				}
				
				#line 224 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
				return  ~ (min) ;
			}
			#line default
		}
		
		
		public static void @remove<T>(T[] a, int length, int pos) {
			unchecked {
				#line 230 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
				global::System.Array.Copy(((global::System.Array) (a) ), ((int) (( pos + 1 )) ), ((global::System.Array) (a) ), ((int) (pos) ), ((int) (( ( length - pos ) - 1 )) ));
				a[( length - 1 )] = default(T);
			}
			#line default
		}
		
		
		public static void insert<T>(ref T[] a, int length, int pos, T x) {
			unchecked {
				#line 236 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
				int capacity = ( a as global::System.Array ).Length;
				if (( pos == length )) {
					#line 239 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
					if (( capacity == length )) {
						#line 241 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
						T[] newarr = new T[( (( length << 1 )) + 1 )];
						( a as global::System.Array ).CopyTo(((global::System.Array) (newarr) ), ((int) (0) ));
						#line 243 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
						a = ((T[]) (newarr) );
					}
					
				}
				else if (( pos == 0 )) {
					#line 248 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
					if (( capacity == length )) {
						#line 250 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
						T[] newarr1 = new T[( (( length << 1 )) + 1 )];
						global::System.Array.Copy(((global::System.Array) (a) ), ((int) (0) ), ((global::System.Array) (newarr1) ), ((int) (1) ), ((int) (length) ));
						#line 252 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
						a = ((T[]) (newarr1) );
					}
					else {
						#line 256 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
						global::System.Array.Copy(((global::System.Array) (a) ), ((int) (0) ), ((global::System.Array) (a) ), ((int) (1) ), ((int) (length) ));
					}
					
				}
				else if (( capacity == length )) {
					#line 263 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
					T[] newarr2 = new T[( (( length << 1 )) + 1 )];
					global::System.Array.Copy(((global::System.Array) (a) ), ((int) (0) ), ((global::System.Array) (newarr2) ), ((int) (0) ), ((int) (pos) ));
					#line 265 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
					global::System.Array.Copy(((global::System.Array) (a) ), ((int) (pos) ), ((global::System.Array) (newarr2) ), ((int) (( pos + 1 )) ), ((int) (( length - pos )) ));
					a = ((T[]) (newarr2) );
				}
				else {
					#line 270 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
					global::System.Array.Copy(((global::System.Array) (a) ), ((int) (pos) ), ((global::System.Array) (a) ), ((int) (( pos + 1 )) ), ((int) (( length - pos )) ));
					global::System.Array.Copy(((global::System.Array) (a) ), ((int) (0) ), ((global::System.Array) (a) ), ((int) (0) ), ((int) (pos) ));
				}
				
				#line 274 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
				a[pos] = x;
			}
			#line default
		}
		
		
		public static global::haxe.lang.FieldHashConflict getHashConflict(global::haxe.lang.FieldHashConflict head, int hash, string name) {
			#line 345 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
			while (( head != null )) {
				#line 346 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
				if (( ( head.hash == hash ) && string.Equals(head.name, name) )) {
					#line 347 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
					return head;
				}
				
				#line 349 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
				head = head.next;
			}
			
			#line 351 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
			return null;
		}
		#line default
		
		public static void setHashConflict(ref global::haxe.lang.FieldHashConflict head, int hash, string name, object @value) {
			#line 355 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
			global::haxe.lang.FieldHashConflict node = head;
			while (( node != null )) {
				#line 357 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
				if (( ( node.hash == hash ) && string.Equals(node.name, name) )) {
					#line 358 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
					node.@value = @value;
					return;
				}
				
				#line 361 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
				node = ((global::haxe.lang.FieldHashConflict) (node.next) );
			}
			
			#line 363 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
			head = ((global::haxe.lang.FieldHashConflict) (new global::haxe.lang.FieldHashConflict(hash, name, @value, ((global::haxe.lang.FieldHashConflict) (head) ))) );
		}
		#line default
		
		public static bool deleteHashConflict(ref global::haxe.lang.FieldHashConflict head, int hash, string name) {
			#line 368 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
			if (( head == null )) {
				#line 369 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
				return false;
			}
			
			#line 373 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
			if (( ( head.hash == hash ) && string.Equals(head.name, name) )) {
				#line 374 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
				head = ((global::haxe.lang.FieldHashConflict) (head.next) );
				return true;
			}
			
			#line 379 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
			global::haxe.lang.FieldHashConflict prev = head;
			#line 379 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
			global::haxe.lang.FieldHashConflict node = head.next;
			while (( node != null )) {
				#line 381 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
				if (( ( node.hash == hash ) && string.Equals(node.name, name) )) {
					#line 382 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
					prev.next = node.next;
					return true;
				}
				
				#line 385 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
				node = node.next;
			}
			
			#line 389 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
			return false;
		}
		#line default
		
		public static void addHashConflictNames(global::haxe.lang.FieldHashConflict head, global::Array<object> arr) {
			#line 393 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
			while (( head != null )) {
				#line 394 "C:\\HaxeToolkit\\haxe\\std\\cs\\internal\\FieldLookup.hx"
				arr.push(head.name);
				head = head.next;
			}
			
		}
		#line default
		
	}
}


